...
def measure_response_time(api_url) -> Dict:
...
def test_api_response_time(api_url):
    ...
    result = measure_response_time(api_url)
    
    # Response time should be under 5 seconds
    assert result['response_time'] < 5.0
    assert result['status_code'] == 200

def test_concurrent_requests(api_url):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = list(executor.map(
            lambda _: measure_response_time(api_url), 
            range(NUM_REQUESTS)
        ))
